...
if ( !($_SESSION['userid'] == 1 || $_SESSION['userid'] == 4) ) {
    ...
    if ( isset($_POST['action'])){
        $sql = "SELECT offline_order_id from offline_order WHERE offline_order_id = :offline_order_id ";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(array(
            ':offline_order_id' => $_POST['offline_order_id']));
        $offline_order_id = $stmt->fetch(PDO::FETCH_ASSOC);
    
        if(empty($offline_order_id['offline_order_id'])){
            $_SESSION['error'] = 'Order Tidak Ditemukan';
            header("Location: order_edit.php");
            return;
        }
        else{
            header("Location:order_edit1.php?offline_order_id=".$offline_order_id['offline_order_id']);
            return;
        }
    }
       ...
       <h1>Edit Order</h1>
         ...
         ?>
            ...
            <div class="form-field">
                <label for="offline_order_id">Order Id :</label>
                <input type="text" name="offline_order_id" id="offline_order_id" class= "input" required>			
            </div>
            <div class="form-field">
