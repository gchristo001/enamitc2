...
if (isset($_POST['itemid'])){
  $sql = "DELETE FROM items WHERE itemid = :zip";
  $stmt = $pdo->prepare($sql);
  $stmt->execute(array(':zip' => $_POST['itemid']));
  $_SESSION['success'] = 'Record deleted';
  $redirect = 'Location: delete_item.php?itemid='.$_POST['itemid'];
  header( $redirect ) ;
  return;
}
     ...
     <div class="input-data">
       ...
       <h2>Delete Data</h2>
         ...
         <div class="form-field" >
						...
						<input type="text" name="name" id="name" class= "input" disabled value="<?= $edited['name'] ?>">
						<small></small>
  			 </div>
  			 <div class="form-field">
  						<label for="supplier">Supplier :</label>
              <select id="supplier" name="supplier"class= "input" disabled >
              ...
              <select id="category" name="category"  class= "input" disabled value="<?= $edited['category'] ?>">
              ...
              <select id="color" name="color"  class= "input" disabled value="<?= $edited['color'] ?>">
         ...
         <div class="form-field">
              ...
              <input type="text" id="size" name="size" class= "input" disabled value="<?= $edited['size'] ?>">
  						<small></small>
  			 </div>
         <div class="form-field">
  						<label for="weight">Weight :</label>
              <input type="text" id="weight" name="weight" class= "input" disabled value="<?= $edited['weight'] ?>">
  						<small></small>
  			 </div>
         <div class="form-field">
  						<label for="price">Price :</label>
              <input type="text" id="price" name="price" class= "input" disabled value="<?= $edited['price'] ?>">
  						<small></small>
  			 </div>
         <div class="form-field">
              <label for="image">Item Image :</label>
  						<input type="file" id="image" name="image" accept="image/png, image/jpeg" class="input" disabled >
  			 </div>
  			 <div class="form-field">
              <input type="hidden" name="itemid" value="<?= $edited['itemid'] ?>">
  						<input id="Reset" type="reset" value="Clear" class= "input">
  						<input id="Submit" type="submit" value="Delete" class= "input">
  			 </div>
       </form>
       <div class="show-data">
